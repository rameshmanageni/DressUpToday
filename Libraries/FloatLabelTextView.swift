...
@IBDesignable class FloatLabelTextView: UITextView {
	...
	let placeholderTextColor = UIColor.mezukaBorderGray
	...
	var titleFont:UIFont = UIFont(name: "SFUIText-Medium", size: 12)! {
		didSet {
	...
	@IBInspectable var titleTextColour:UIColor = .mezukaDarkGray {
		didSet {
			if !isFirstResponder {
	...
	@IBInspectable var titleActiveTextColour:UIColor = .mezukaDarkGray {
		didSet {
			if isFirstResponder {
    ...
    var hintLabelY:CGFloat = 10
	override func layoutSubviews() {
		super.layoutSubviews()
		adjustTopTextInset()
		hintLabel.alpha = text.isEmpty ? 1.0 : 0.0
		let r = textRect()
		hintLabel.frame = CGRect(x:r.origin.x, y:r.origin.y - hintLabelY, width:hintLabel.frame.size.width, height:hintLabel.frame.size.height)
	...
	fileprivate func showTitle(_ animated:Bool) {
		...
		UIView.animate(withDuration: dur, delay:0, options: [UIViewAnimationOptions.beginFromCurrentState, UIViewAnimationOptions.curveEaseOut], animations:{
            ...
            self.title.textColor = .mezukaDarkGray
			}, completion:nil)
	}
